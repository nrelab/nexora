...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/theonlyhennygod/nexora/security/advisories/new)
...
NexOra implements defense-in-depth security:
...
NexOra Docker images follow CIS Docker Benchmark best practices:
...
docker inspect --format='{{.Config.User}}' nexora
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace nexora gateway
```
