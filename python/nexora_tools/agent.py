...
SYSTEM_PROMPT = """You are NexOra, an AI assistant with tool access. Use tools to accomplish tasks.
...
class NexoraAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "NexoraAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await NexoraAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a NexOra agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured NexoraAgent instance
