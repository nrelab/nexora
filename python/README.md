...
Python companion package for [NexOra](https://github.com/nrelab/nexora) — LangGraph-based tool calling for consistent LLM agent execution.
...
```bash
pip install nexora-tools[discord]
```
...
# Run the CLI
nexora-tools "List files in the current directory"
...
| Feature | Rust NexOra | nexora-tools |
|---------|---------------|----------------|
...
Use **Rust NexOra** for production edge deployments. Use **nexora-tools** when you need guaranteed tool calling consistency or Python ecosystem integration.
