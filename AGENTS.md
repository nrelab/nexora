# AGENTS.md — NexOra Agent Engineering Protocol
...
NexOra is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use NexOra-native labels only (`NexOraAgent`, `nexora_user`, `nexora_node`).
...
- If identity-like context is unavoidable, use NexOra-scoped roles/labels only (for example: `NexOraAgent`, `NexOraOperator`, `nexora_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `NexOraAgent`, `NexOraOperator`, `NexOraMaintainer`, `nexora_user`
    - service/runtime labels: `nexora_bot`, `nexora_service`, `nexora_runtime`, `nexora_node`
    - environment labels: `nexora_project`, `nexora_workspace`, `nexora_channel`
