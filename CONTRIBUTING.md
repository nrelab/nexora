...
Thanks for your interest in contributing to NexOra! This guide will help you get started.
...
NexOra supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.nexora/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.nexora/.secret_key` with restricted permissions
    - Use `nexora onboard` for guided setup
...
3. Generic env vars (`NEXORA_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `NEXORA_PROVIDER` / `PROVIDER`
- `NEXORA_MODEL`
...
- `~/.nexora/.secret_key` or similar key files
...
- If identity-like wording is required, use NexOra-centric labels only (for example: `NexOraAgent`, `NexOraOperator`, `nexora_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use NexOra/project-native roles instead of personal or real-world identities.
...
NexOra's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use NexOra-native identifiers only (`NexOraAgent`, `nexora_user`, `nexora_node`).
...
- **Good identity-like label**: `NexOraAgent`, `nexora_user`, `nexora_node`
...
- [ ] Any required identity-like wording uses NexOra/project-native labels only
