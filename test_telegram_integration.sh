#!/bin/bash
# NexOra Telegram Integration Test Suite
...
if [ -f "target/release/nexora" ]; then
    ...
    SIZE_BYTES=$(stat -f%z target/release/nexora 2>/dev/null || stat -c%s target/release/nexora)
...
CONFIG_PATH="$HOME/.nexora/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'nexora onboard' first"
    fi
else
    warn "No config file found - run 'nexora onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/nexora channel doctor 2>&1 || true)
END_TIME=$(date +%s)
