# Frictionless Security: Nex Impact on Wizard
...
[INFO] ✓ Audit logging enabled (~/.config/nexora/audit.log)
...
```bash
# Check what's active
$ nexora security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/nexora/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ nexora config set security.sandbox.enabled false

# Enable specific backend
$ nexora config set security.sandbox.backend firejail

# Adjust limits
$ nexora config set security.resources.max_memory_mb 2048
```
...
```bash
$ nexora onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Nex impact on wizard** — stays 9 steps, < 60 seconds
✅ **Nex new prompts** — silent auto-detection
✅ **Nex breaking changes** — backward compatible
✅ **Opt-out available** — explicit config flags
✅ **Status visibility** — `nexora security --status`
