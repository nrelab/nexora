# Network Deployment — NexOra on Raspberry Pi and Local Network

This document covers deploying NexOra on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | NexOra polls Telegram API; works from anywhere |
...
**Key:** Telegram, Discord, and Slack use **long-polling** — NexOra makes outbound requests. No port forwarding or public IP required.

---

## 2. NexOra on Raspberry Pi
...
- Telegram channel works: NexOra polls Telegram API (outbound)
...
```bash
nexora daemon --host 0.0.0.0 --port 8080
```
...
2. Start a tunnel:
   ...
   ```
   Or use `nexora tunnel` (see tunnel docs).

3. NexOra will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- NexOra calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `nexora daemon` — Telegram channel starts automatically.
...
- [ ] Run `nexora daemon --host 127.0.0.1 --port 8080` (Telegram works without 0.0.0.0)
