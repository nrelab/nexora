# Adding Boards and Tools — NexOra Hardware Guide

This guide explains how to add new hardware boards and custom tools to NexOra.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.nexora/config.toml)
nexora peripheral add nucleo-f401re /dev/ttyACM0
nexora peripheral add arduino-uno /dev/cu.usbmodem12345
nexora peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, NexOra can index PDF files:
...
2. **Add to config** — `nexora peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `nexora peripheral list` | List configured boards |
| `nexora peripheral add <board> <path>` | Add board (writes config) |
| `nexora peripheral flash` | Flash Arduino firmware |
| `nexora peripheral flash-nucleo` | Flash Nucleo firmware |
| `nexora hardware discover` | List USB devices |
| `nexora hardware info` | Chip info via probe-rs |
