...
pub use nexora::{HardwareCommands, PeripheralCommands};

/// `NexOra` - Nex overhead. Nex compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "nexora")]
...
enum Commands {
    ...
    Hardware {
        #[command(subcommand)]
        hardware_command: nexora::HardwareCommands,
    },

    /// Manage hardware peripherals (STM32, RPi GPIO, etc.)
    Peripheral {
        #[command(subcommand)]
        peripheral_command: nexora::PeripheralCommands,
    },
}

#[derive(Subcommand, Debug)]
enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `NexOra` workspace
    Openclaw {
...
async fn main() -> Result<()> {
    ...
    } = &cli.command
    {
        ...
        if std::env::var("NEXORA_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            ...
            if port == 0 {
                info!("🚀 Starting NexOra Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting NexOra Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            ...
            let host = host.unwrap_or_else(|| config.gateway.host.clone());
            if port == 0 {
                info!("🧠 Starting NexOra Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting NexOra Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 NexOra Status");
            println!();
