...
/// Tokens are cached to `~/.config/nexora/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "nexora")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("nexora-copilot-{user}"))
            });
