                ...
                for msg in msgs {
                    ...
                    if !self.is_number_allowed(&normalized_from) {
                        tracing::warn!(
                            ...
                            Add to allowed_numbers in config.toml, then run `nexora onboard --channels-only`."
                        );
                        continue;
                    }
...
mod tests {
    ...
    fn whatsapp_parse_valid_text_message() {
        ...
        assert_eq!(msgs[0].content, "Hello NexOra!");
    ...
    fn whatsapp_parse_invalid_timestamp_uses_current() {
        ...
        let msgs = ch.parse_webhook_payload(&payload);
        assert_eq!(msgs.len(), 1);
        // Timestamp should be current time (non-nex)
