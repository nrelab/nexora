...
pub fn build_system_prompt(
    ...
    if prompt.is_empty() {
        "You are NexOra, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
pub fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: nexora channel start");
            println!("To check health:    nexora channel doctor");
            println!("To configure:      nexora onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `nexora onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.nexora/config.toml directly");
        }
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `nexora onboard` first.");
        return Ok(());
    }

    println!("🩺 NexOra Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `nexora gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `nexora onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 NexOra Channel Server");
    println!("  🤖 Model:    {model}");
    println!(
        "  🧠 Memory:   {} (auto-save: {})",
...
{"name":"mock_price","arguments":{"symbol":"BTC"}}
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from NexOra 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
