...
const SERVICE_LABEL: &str = "com.nexora.daemon";
...
fn start(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run_checked(Command::new("systemctl").args(["--user", "start", "nexora.service"]))?;
        println!("✅ Service started");
        Ok(())
    } else {
        let _ = config;
...
fn stop(config: &Config) -> Result<()> {
    if cfg!(target_os = "macos") {
        ...
        let _ = run_checked(Command::new("systemctl").args(["--user", "stop", "nexora.service"]));
        println!("✅ Service stopped");
        Ok(())
    } else {
        let _ = config;
...
"#,
    ...
    println!("   Start with: nexora service start");
    Ok(())
}
...
fn install_linux(config: &Config) -> Result<()> {
    ...
    let exe = std::env::current_exe().context("Failed to resolve current executable")?;
    let unit = format!(
        "[Unit]\nDescription=NexOra daemon\nAfter=network.target\n\n[Service]\nType=simple\nExecStart={} daemon\nRestart=always\nRestartSec=3\n\n[Install]\nWantedBy=default.target\n",
        exe.display()
    );

    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "nexora.service"]));
...
mod tests {
    ...
    #[test]
    fn run_checked_errors_on_non_nex_status() {
        let err = run_checked(Command::new("sh").args(["-lc", "exit 17"]))
            .expect_err("non-nex exit should error");
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/nexora.service"));
    }
}
