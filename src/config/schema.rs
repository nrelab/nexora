...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "nexora".
    #[serde(default)]
...
pub struct CloudflareTunnelConfig {
    /// Cloudflare Tunnel token (from Nex Trust dashboard)
    pub token: String,
}
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `NEXORA_WHATSAPP_APP_SECRET` environment variable
    #[serde(default)]
...
/// How NexOra receives events from Feishu / Lark.
///
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to nexora dir)
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            workspace_dir: nexora_dir.join("workspace"),
            config_path: nexora_dir.join("config.toml"),
            api_key: None,
            api_url: None,
...
fn default_config_and_workspace_dirs() -> Result<(PathBuf, PathBuf)> {
    ...
    let config_dir = home.join(".nexora");
...
impl Config {
    pub fn load_or_init() -> Result<Self> {
        // Resolve workspace first so config loading can follow NEXORA_WORKSPACE.
        let (nexora_dir, workspace_dir) = match std::env::var("NEXORA_WORKSPACE") {
        ...
        let config_path = nexora_dir.join("config.toml");

        fs::create_dir_all(&nexora_dir).context("Failed to create config directory")?;
        ...
        if config_path.exists() {
            ...
            let store = crate::security::SecretStore::new(&nexora_dir, config.secrets.encrypt);
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: NEXORA_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("NEXORA_API_KEY").or_else(|_| std::env::var("API_KEY")) {
            if !key.is_empty() {
        ...
        // Provider: NEXORA_PROVIDER or PROVIDER
        if let Ok(provider) =
            std::env::var("NEXORA_PROVIDER").or_else(|_| std::env::var("PROVIDER"))
        {
        ...
        // Model: NEXORA_MODEL
        if let Ok(model) = std::env::var("NEXORA_MODEL") {
            if !model.is_empty() {
                self.default_model = Some(model);
            }
        }

        // Workspace directory: NEXORA_WORKSPACE
        if let Ok(workspace) = std::env::var("NEXORA_WORKSPACE") {
            if !workspace.is_empty() {
                self.workspace_dir = PathBuf::from(workspace);
            }
        }

        // Gateway port: NEXORA_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("NEXORA_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
        {
        ...
        // Gateway host: NEXORA_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("NEXORA_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
        {
            if !host.is_empty() {
        ...
        // Allow public bind: NEXORA_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("NEXORA_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Temperature: NEXORA_TEMPERATURE
        if let Ok(temp_str) = std::env::var("NEXORA_TEMPERATURE") {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        let store = crate::security::SecretStore::new(nexora_dir, self.secrets.encrypt);
...
"#;
    ...
    fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("nexora_test_config");
    ...
    fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "nexora_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).unwrap();
    ...
    fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("nexora_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).unwrap();
...
"#;
    ...
    fn env_override_api_key() {
        ...
        std::env::set_var("NEXORA_API_KEY", "sk-test-env-key");
        config.apply_env_overrides();
        assert_eq!(config.api_key.as_deref(), Some("sk-test-env-key"));

        std::env::remove_var("NEXORA_API_KEY");
    }
    ...
    fn env_override_provider() {
        ...
        std::env::set_var("NEXORA_PROVIDER", "anthropic");
        config.apply_env_overrides();
        assert_eq!(config.default_provider.as_deref(), Some("anthropic"));

        std::env::remove_var("NEXORA_PROVIDER");
    }
    ...
    fn env_override_model() {
        ...
        std::env::set_var("NEXORA_MODEL", "gpt-4o");
        config.apply_env_overrides();
        assert_eq!(config.default_model.as_deref(), Some("gpt-4o"));

        std::env::remove_var("NEXORA_MODEL");
    }
    ...
    fn env_override_workspace() {
        ...
        std::env::set_var("NEXORA_WORKSPACE", "/custom/workspace");
        config.apply_env_overrides();
        assert_eq!(config.workspace_dir, PathBuf::from("/custom/workspace"));

        std::env::remove_var("NEXORA_WORKSPACE");
    }
    ...
    fn load_or_init_workspace_override_uses_workspace_root_for_config() {
        let _env_guard = env_override_test_guard();
        let temp_home =
            std::env::temp_dir().join(format!("nexora_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("profile-a");

        let original_home = std::env::var("HOME").ok();
        std::env::set_var("HOME", &temp_home);
        std::env::set_var("NEXORA_WORKSPACE", &workspace_dir);
    ...
    fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let legacy_config_path = temp_home.join(".nexora").join("config.toml");
    ...
    fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".nexora");
...
"#,
    ...
    fn env_override_empty_values_ignored() {
        ...
        std::env::set_var("NEXORA_PROVIDER", "");
        config.apply_env_overrides();
    ...
    fn env_override_gateway_port() {
        ...
        std::env::set_var("NEXORA_GATEWAY_PORT", "8080");
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        std::env::remove_var("NEXORA_GATEWAY_PORT");
    }
    ...
    fn env_override_gateway_host() {
        ...
        std::env::set_var("NEXORA_GATEWAY_HOST", "0.0.0.0");
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        std::env::remove_var("NEXORA_GATEWAY_HOST");
    }
    ...
    fn env_override_temperature() {
        ...
        std::env::set_var("NEXORA_TEMPERATURE", "0.5");
        config.apply_env_overrides();
        assert!((config.default_temperature - 0.5).abs() < f64::EPSILON);

        std::env::remove_var("NEXORA_TEMPERATURE");
    }
    ...
    fn env_override_temperature_out_of_range_ignored() {
        ...
        std::env::set_var("NEXORA_TEMPERATURE", "3.0");
        config.apply_env_overrides();
        assert!(
