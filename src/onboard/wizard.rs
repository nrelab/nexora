...
const BANNER: &str = r"
    ...
    Nex overhead. Nex compromise. 100% Rust. 100% Agnostic.
...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to NexOra — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 9, "Channels (How You Talk to NexOra)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("NEXORA_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to NexOra)");
...
// ── Quick setup (nex prompts) ───────────────────────────────────

/// Non-interactive setup: generates a sensible default config instantly.
/// Use `nexora onboard` or `nexora onboard --api-key sk-... --provider openrouter --memory sqlite|lucid`.
/// Use `nexora onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    let workspace_dir = nexora_dir.join("workspace");
    ...
    if credential_override.is_none() {
        ...
        println!("    2. Or edit:           ~/.nexora/config.toml");
        println!("    3. Chat:              nexora agent -m \"Hello!\"");
        println!("    4. Gateway:           nexora gateway");
    } else {
        println!("    1. Chat:     nexora agent -m \"Hello!\"");
        println!("    2. Gateway:  nexora gateway");
        println!("    3. Status:   nexora status");
    }
    println!();
...
pub fn run_models_refresh(
    ...
    if !force {
        if let Some(cached) = load_cached_models_for_provider(
            ...
            println!();
            println!(
                "Tip: run `nexora models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".nexora");
...
fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("NexOra works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("NexOra will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how NexOra connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("NexOra uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("NexOra can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("NexOra can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should NexOra interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how NexOra stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to NexOra from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to NexOra from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to NexOra from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to NexOra from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("NexOra reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("nexora-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via NEXORA_WHATSAPP_APP_SECRET env var
                    allowed_numbers,
                });
            }
            6 => {
...
fn setup_tunnel() -> Result<crate::config::TunnelConfig> {
    ...
    let options = vec![
        ...
        "Cloudflare Tunnel — Nex Trust, free tier",
    ...
    let config = match choice {
        1 => {
            println!();
            print_bullet("Get your tunnel token from the Cloudflare Nex Trust dashboard.");
            let token: String = Input::new()
...
fn scaffold_workspace(workspace_dir: &Path, ctx: &ProjectContext) -> Result<()> {
    ...
    let soul = format!(
         ...
         **Earn trust through competence.** Your human gave you access\n\
         ...
         ## Identity\n\n\
         You are **{agent}**. Built in Rust. 3MB binary. Nex bloat.\n\
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("NexOra is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("nexora channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("nexora agent -m \"Hello, NexOra!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("nexora agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("nexora status").yellow());
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** NexOra"),
            "should default agent name to NexOra"
        );
    ...
    fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("NexOra-v2"));
    }
