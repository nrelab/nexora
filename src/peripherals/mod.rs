...
/// Handle `nexora peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: nexora peripheral add <board> <path>");
                println!("  Example: nexora peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
