//! Flash NexOra Arduino firmware via arduino-cli.
//!
...
/// NexOra Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/nexora-arduino/nexora-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "nexora-arduino";
...
/// Flash NexOra firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("nexora_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling NexOra Arduino firmware...");
    ...
    println!("NexOra firmware flashed successfully.");
