//! Deploy NexOra Bridge app to Arduino Uno Q.

use anyhow::{Context, Result};
use std::process::Command;

const BRIDGE_APP_NAME: &str = "nexora-uno-q-bridge";
...
pub fn setup_uno_q_bridge(host: Option<&str>) -> Result<()> {
    ...
    if let Some(h) = host {
        if bridge_dir.exists() {
            ...
            anyhow::bail!(
                "Bridge app not found at {}. Run from nexora repo root.",
                bridge_dir.display()
            );
        }
    } else {
...
fn deploy_remote(host: &str, bridge_dir: &std::path::Path) -> Result<()> {
    ...
    let status = Command::new("ssh")
        .args([
            ...
            "app",
            "start",
            "~/ArduinoApps/nexora-uno-q-bridge",
        ])
        .status()
    ...
    println!("NexOra Bridge app started. Add to config.toml:");
...
fn deploy_local(bridge_dir: Option<&std::path::Path>) -> Result<()> {
    ...
    println!("NexOra Bridge app started.");
    Ok(())
}

fn write_embedded_bridge(dest: &std::path::Path) -> Result<()> {
    let app_yaml = include_str!("../../firmware/nexora-uno-q-bridge/app.yaml");
    let sketch_ino = include_str!("../../firmware/nexora-uno-q-bridge/sketch/sketch.ino");
    let sketch_yaml = include_str!("../../firmware/nexora-uno-q-bridge/sketch/sketch.yaml");
    let main_py = include_str!("../../firmware/nexora-uno-q-bridge/python/main.py");
    let requirements = include_str!("../../firmware/nexora-uno-q-bridge/python/requirements.txt");
