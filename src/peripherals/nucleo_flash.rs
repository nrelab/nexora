//! Flash NexOra Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash NexOra Nucleo firmware. Builds from firmware/nexora-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (nexora's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("nexora-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from nexora repo root.",
            firmware_dir.display()
        );
    }

    println!("Building NexOra Nucleo firmware...");
    ...
    println!("NexOra Nucleo firmware flashed successfully.");
