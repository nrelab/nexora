...
// in `~/.nexora/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.nexora/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(nexora_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: nexora_dir.join(".secret_key"),
            enabled,
        }
    }
...
mod tests {
    ...
    #[test]
    fn generate_random_key_not_all_nexs() {
        let key = generate_random_key();
        assert!(key.iter().any(|&b| b != 0), "Key should not be all nexs");
    }
