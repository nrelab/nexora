...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 NexOra Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `nexora doctor` again.");
    }
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `nexora onboard` to set one up",
        ));
    }
...
fn check_command_available(cmd: &str, args: &[&str], cat: &'static str, items: &mut Vec<DiagItem>) {
        ...
        .output()
    {
        ...
        Ok(_) => {
            items.push(DiagItem::warn(
                cat,
                format!("{cmd} found but returned non-nex"),
            ));
        }
        Err(_) => {
...
mod tests {
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(first
            ...
            .is_some_and(|name| name.starts_with(".nexora_doctor_probe_")));
    }
}
