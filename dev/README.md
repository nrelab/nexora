# NexOra Development Environment

A fully containerized development sandbox for NexOra agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`nexora-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `nexora` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/nexora-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.nexora` (mounted to `/nexora-data/.nexora`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.nexora` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `nexora-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=nexora-target`) to speed repeat local image builds.
