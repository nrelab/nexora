...
<p align="center">
  <strong>Nex overhead. Nex compromise. 100% Rust. 100% Agnostic.</strong><br>
...
## Benchmark Snapshot (NexOra vs OpenClaw)

Local machine quick benchmark (macOS arm64, Feb 2026) normalized for 0.8GHz edge hardware.

| | OpenClaw | NanoBot | PicoClaw | NexOra 🦀 |
|---|---|---|---|---|
...
> Notes: NexOra results measured with `/usr/bin/time -l` on release builds. OpenClaw requires Node.js runtime (~390MB overhead). PicoClaw and NexOra are static binaries.

<p align="center">
  <img src="nex-claw.jpeg" alt="NexOra vs OpenClaw Comparison" width="800" />
</p>

Reproduce NexOra numbers locally:

```bash
cargo build --release
ls -lh target/release/nexora

/usr/bin/time -l target/release/nexora --help
/usr/bin/time -l target/release/nexora status
```
...
```bash
git clone https://github.com/nrelab/nexora.git
cd nexora
...
nexora onboard --api-key sk-... --provider openrouter

# Or interactive wizard
nexora onboard --interactive

# Or quickly repair channels/allowlists only
nexora onboard --channels-only

# Chat
nexora agent -m "Hello, NexOra!"

# Interactive mode
nexora agent

# Start the gateway (webhook server)
nexora gateway                # default: 127.0.0.1:8080
nexora gateway --port 0       # random port (security hardened)
...
nexora channel doctor

# Get integration setup details
nexora integrations info Telegram
...
nexora migrate openclaw --dry-run
nexora migrate openclaw
```
...
Every subsystem is a **trait** — swap implementations with a config change, nex code changes.

<p align="center">
  <img src="docs/architecture.svg" alt="NexOra Architecture" width="900" />
</p>
...
When an unsupported `runtime.kind` is configured, NexOra now exits with a clear error instead of silently falling back to native.

### Memory System (Full-Stack Search Engine)

All custom, nex external dependencies — no Pinecone, no Elasticsearch, no LangChain:
...
# NEXORA_LUCID_CMD=/usr/local/bin/lucid   # default: lucid
# NEXORA_LUCID_BUDGET=200                 # default: 200
# NEXORA_LUCID_LOCAL_HIT_THRESHOLD=3      # local hit count to skip external recall
# NEXORA_LUCID_RECALL_TIMEOUT_MS=120      # low-latency budget for lucid context recall
# NEXORA_LUCID_STORE_TIMEOUT_MS=800        # async sync timeout for lucid store
# NEXORA_LUCID_FAILURE_COOLDOWN_MS=15000   # cooldown after lucid failure to avoid repeated slow attempts
```

## Security

NexOra enforces security at **every layer** — not just the sandbox. It passes all items from the community security checklist.
...
> **Run your own nmap:** `nmap -p 1-65535 <your-host>` — NexOra binds to localhost only, so nothing is exposed unless you explicitly configure a tunnel.
...
For non-text replies, NexOra can send Telegram attachments when the assistant includes markers:
...
6. **Test:** Send a message to your WhatsApp Business number — NexOra will respond via the LLM.

## Configuration

Config: `~/.nexora/config.toml` (created by `onboard`)
...
#   "metadata": {"session_name": "...", "source": "nexora.browser", "version": "..."}
# }
...
## Python Companion Package (`nexora-tools`)

For LLM providers with inconsistent native tool calling (e.g., GLM-5/Zhipu), NexOra ships a Python companion package with **LangGraph-based tool calling** for guaranteed consistency:

```bash
pip install nexora-tools
```

```python
from nexora_tools import create_agent, shell, file_read
...
NexOra supports **identity-agnostic** AI personas through two formats:
...
[AIEOS](https://aieos.org) is a standardization framework for portable AI identity. NexOra supports AIEOS v1.1 JSON payloads, allowing you to:
...
NexOra is configured to use `rustls` for HTTP/TLS dependencies; `--locked` keeps the transitive graph deterministic on fresh environments.
...
- New `Skill` → `~/.nexora/workspace/skills/<name>/`

---

**NexOra** — Nex overhead. Nex compromise. Deploy anywhere. Swap anything. 🦀

## Star History

<p align="center">
  <a href="https://www.star-history.com/#nrelab/nexora&Date">
    <img src="https://api.star-history.com/svg?repos=nrelab/nexora&type=Date" alt="Star History Chart" />
  </a>
</p>
